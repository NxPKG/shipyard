...
REMOTE="$(get_remote)"
CHARTS_PATH="${CHARTS_PATH:-$GOPATH/src/github.com/khulnasoft/charts}"
CHARTS_TOOL="prepare_artifacts.sh"
RELEASES_URL="https://github.com/khulnasoft/shipyard/releases"
VERSION=""

usage() {
    echo "usage: $0 [VERSION]"
    echo "CHARTS_REPO Path to local copy of github.com/khulnasoft/charts"
    echo
...
handle_args() {
    ...
    if [[ ! -e "$CHARTS_PATH/$CHARTS_TOOL" ]]; then
        usage
        common::exit 1 "CHARTS_PATH='$CHARTS_PATH' invalid. Clone from github.com/khulnasoft/charts"
    fi
