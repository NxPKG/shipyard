...
import (
	...
	. "github.com/khulnasoft/shipyard/test/ginkgo-ext"
	"github.com/khulnasoft/shipyard/test/helpers"
	"github.com/khulnasoft/shipyard/test/helpers/constants"
)
...
var kvstoreChaosTests = func() {
	...
	It("Delete event on KVStore with CIDR identities", func() {
		...
		CIDRIdentities := vm.Exec(fmt.Sprintf(`cilium-dbg identity list -o json| %s`, jqFilter))
		...
		newCIDRIdentities := vm.Exec(fmt.Sprintf(`cilium-dbg identity list -o json| %s`, jqFilter))
