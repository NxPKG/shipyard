...
function gather_files {
  ...
  local CILIUM_ROOT="src/github.com/khulnasoft/shipyard"
...
function copy_files_vm {
  ...
  vagrant ssh $VM_NAME -c 'sudo -E bash -c "journalctl --no-pager -u cilium > /home/vagrant/go/src/github.com/khulnasoft/shipyard/test/envoy/cilium-files/cilium-logs && chmod a+r /home/vagrant/go/src/github.com/khulnasoft/shipyard/test/envoy/cilium-files/cilium-logs"'
  vagrant ssh $VM_NAME -c 'sudo -E bash -c "journalctl --no-pager -u cilium-docker > /home/vagrant/go/src/github.com/khulnasoft/shipyard/test/envoy/cilium-files/cilium-docker-logs && chmod a+r /home/vagrant/go/src/github.com/khulnasoft/shipyard/test/envoy/cilium-files/cilium-docker-logs"'

  log "listing all logs that will be gathered from $VM_NAME"
  vagrant ssh $VM_NAME -c 'ls -altr /home/vagrant/go/src/github.com/khulnasoft/shipyard/test/envoy/cilium-files'

  log "copying logs from $VM_NAME onto VM host for accessibility after VM is destroyed"
  scp -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -r -P ${PORT} -i ${ID_FILE} vagrant@127.0.0.1:/home/vagrant/go/src/github.com/khulnasoft/shipyard/${FILES_DIR} ${WORKSPACE}/cilium-files-${VM_NAME}
}
