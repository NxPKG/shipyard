...
TEST_LDFLAGS=-ldflags "-X github.com/khulnasoft/shipyard/pkg/kvstore.consulDummyAddress=https://consul:8443 \
	-X github.com/khulnasoft/shipyard/pkg/kvstore.etcdDummyAddress=http://etcd:4002 \
	-X github.com/khulnasoft/shipyard/pkg/datapath.DatapathSHA256=e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855"

TEST_UNITTEST_LDFLAGS=-ldflags "-X github.com/khulnasoft/shipyard/pkg/datapath.DatapathSHA256=e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855"
...
generate-cov: ## Generate HTML coverage report at coverage-all.html.
	...
	$(QUIET) grep -Ev '(^github.com/khulnasoft/shipyard/api/v1)|(generated.deepcopy.go)|(^github.com/khulnasoft/shipyard/pkg/k8s/client/)' \
...
cscope.files: ## Generate cscope.files with the list of all files to generate ctags for.
	...
	@go list -f "{{ \$$p := .ImportPath }} \
		...
		| sed 's#github.com/khulnasoft/shipyard/##g' | sort | uniq > cscope.files
...
define generate_deepequal
	$(GO) run github.com/khulnasoft/deepequal-gen \
...
define generate_k8s_protobuf
	...
	$(GO) run k8s.io/code-generator/cmd/go-to-protobuf \
		...
		--drop-embedded-fields="github.com/khulnasoft/shipyard/pkg/k8s/slim/k8s/apis/meta/v1.TypeMeta" \
		--proto-import="$(PWD)" \
...
github.com/khulnasoft/shipyard/pkg/k8s/slim/k8s/api/core/v1
github.com/khulnasoft/shipyard/pkg/k8s/slim/k8s/api/discovery/v1
github.com/khulnasoft/shipyard/pkg/k8s/slim/k8s/api/discovery/v1beta1
github.com/khulnasoft/shipyard/pkg/k8s/slim/k8s/api/networking/v1
github.com/khulnasoft/shipyard/pkg/k8s/slim/k8s/apis/apiextensions/v1
github.com/khulnasoft/shipyard/pkg/k8s/slim/k8s/apis/meta/v1
github.com/khulnasoft/shipyard/pkg/k8s/slim/k8s/apis/meta/v1beta1
github.com/khulnasoft/shipyard/pkg/k8s/slim/k8s/apis/util/intstr
endef
...
generate-k8s-api: ## Generate Cilium k8s API client, deepcopy and deepequal Go sources.
	...
	$(eval DEEPEQUAL_PACKAGES := $(shell grep "\+deepequal-gen" -l -r --include \*.go --exclude-dir 'vendor' . | xargs dirname {} | sort | uniq | grep -x -v '.' | sed 's|\.\/|github.com/khulnasoft/shipyard\/|g'))
	$(QUIET) $(call generate_deepequal,${DEEPEQUAL_PACKAGES},"$(TMPDIR)")

	$(eval DEEPCOPY_PACKAGES := $(shell grep "\+k8s:deepcopy-gen" -l -r --include \*.go --exclude-dir 'vendor' . | xargs dirname {} | sort | uniq | grep -x -v '.' | sed 's|\.\/|github.com/khulnasoft/shipyard\/|g'))
	$(QUIET) $(call generate_deepcopy,${DEEPCOPY_PACKAGES},"$(TMPDIR)")

	$(QUIET) $(call generate_k8s_api,client,github.com/khulnasoft/shipyard/pkg/k8s/slim/k8s/client,github.com/khulnasoft/shipyard/pkg/k8s/slim/k8s/api,"discovery:v1beta1 discovery:v1 networking:v1 core:v1","$(TMPDIR)")
	$(QUIET) $(call generate_k8s_api,client,github.com/khulnasoft/shipyard/pkg/k8s/slim/k8s/apiextensions-client,github.com/khulnasoft/shipyard/pkg/k8s/slim/k8s/apis,"apiextensions:v1","$(TMPDIR)")
	$(QUIET) $(call generate_k8s_api,client$(comma)lister$(comma)informer,github.com/khulnasoft/shipyard/pkg/k8s/client,github.com/khulnasoft/shipyard/pkg/k8s/apis,$(GEN_CRD_GROUPS),"$(TMPDIR)")

	$(QUIET) cp -r "$(TMPDIR)/github.com/khulnasoft/shipyard/." ./
	$(QUIET) rm -rf "$(TMPDIR)"
...
kind-install-cilium: kind-ready ## Install a local Cilium version into the cluster.
	...
	# reinstall here. https://github.com/khulnasoft/shipyard-cli/issues/205
	...
	# https://github.com/khulnasoft/shipyard-cli/issues/1070
	$(CILIUM_CLI) install \
		--chart-directory=$(ROOT_DIR)/install/kubernetes/cilium \
		$(KIND_VALUES_FILES) \
		--version= \
